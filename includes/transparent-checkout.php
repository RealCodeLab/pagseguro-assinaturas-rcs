...
if (method_exists($order, 'get_id')) {
    $pga_order_id = intval($order->get_id());
} else {
    $pga_order_id = intval($order->id);
}
?>
...
<p class="hide_after_payment"><?php echo apply_filters('woocommerce_pagseguro_transparent_checkout_message',
        __('Este pagamento será processado pelo Pagseguro.', 'pagseguro-assinaturas-rcs')); ?></p>
<p><?php echo $plano_description; ?></p>

<form action="" method="post" id="woocommerce-pagseguro-assinaturas-rcs-payment-form">
    <input type="hidden" name="code_pagseguro" id="code_pagseguro"
           value="<?php echo ! empty($_GET['code']) ? $_GET['code'] : ""; ?>"/>
    <input type="hidden" name="order_id" id="woocommerce-pagseguro-order-id" value="<?php echo $pga_order_id; ?>"/>
...
<div class="show_after_payment" style="display:none">
    <h3><?php echo sprintf(__('Seu pedido #%d foi pago com sucesso', 'pagseguro-assinaturas-rcs'),
            $pga_order_id); ?></h3>
