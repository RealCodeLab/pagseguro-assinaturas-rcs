...
class PGA_Compra
{
    ...
    function verificar_mistura_produtos($produto_id)
    {
        ...
        foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
            ...
            if ($add_prod_is_plano != $cart_prod_is_plano) {
                wc_add_notice(__('Desculpe, não é possível assinar planos e comprar outros tipos de produtos ao mesmo tempo.',
                    'pagseguro-assinaturas-rcs'), 'error');
            ...
            if ($cart_prod_is_plano == 'yes' || $add_produto_meta == 'yes') {
                wc_add_notice(__('Desculpe, só é permitido 1 plano no carrinho por vez.', 'pagseguro-assinaturas-rcs'),
                    'error');
    ...
    function pga_woocommerce_product_meta_start()
    {
        ...
        if ($produto_meta['_is_plano'][0] == 'yes') {
            if (isset($produto_meta['_taxa_contratacao']) && $produto_meta['_taxa_contratacao'][0] > 0) {
                ...
                <span class="price">
                    <ins>
                        <span class="amount"><?php echo __('Taxa de contratação: ',
                                    'pagseguro-assinaturas-rcs') . PGA_Utils::format_money_currency($produto_meta['_taxa_contratacao'][0]); ?></span>
                    </ins>
                </span>
                <?php
            }
    ...
    function pga_woocommerce_cart_item_subtotal($subtotal, $cart_item, $cart_item_key)
    {
        ...
        if ($total_taxa > 0 && $produto_meta['_is_plano'][0] == 'yes') {
            $subtotal .= '<br>';
            $subtotal .= PGA_Utils::format_money_currency($total_taxa) . __(' (taxa de contratação)',
    ...
    function pga_woocommerce_cart_subtotal($cart_subtotal, $compound, $cart)
    {
        ...
        if ($this->total_taxa > 0) {
            $cart_subtotal .= '<br>';
            $cart_subtotal .= PGA_Utils::format_money_currency($this->total_taxa) . __(' (taxa de contratação)',
                    'pagseguro-assinaturas-rcs');
        }
    ...
    function pga_woocommerce_my_account_my_orders_actions($actions, $order)
    {
        if ($order->status != 'cancelled' && ! array_key_exists('cancel', $actions)) {
            if (method_exists($order, 'get_id')) {
                $actions['force_cancel']['url'] = add_query_arg('force_cancel', $order->get_id(),
                    PGA_Utils::get_current_url());
            } else {
                $actions['force_cancel']['url'] = add_query_arg('force_cancel', $order->id,
                    PGA_Utils::get_current_url());
            }
    ...
    function plano_description($produto_meta)
    {
        ...
        if ($this->controller->gateway->aviso_produto_plano) {
            return sprintf(__("<span class='plano_descricao' {$style}>{$this->controller->gateway->aviso_produto_plano}</span>"),
                $intervalo_descricao);
        }

        return sprintf(__("<span class='plano_descricao' {$style}>Este produto é um plano. O intervalo de pagamento é %s.</span>"),
            $intervalo_descricao);
    }
